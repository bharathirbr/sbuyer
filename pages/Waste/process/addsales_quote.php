...
$getmanuf = $db->prepare("SELECT * FROM `sales_quote` ORDER BY `PoID` DESC");
$getmanuf->execute();
$lst = $getmanuf->fetch(PDO::FETCH_ASSOC);

if ($lst['PoID'] == '' || $lst['PoID'] == '0') {
    $val = 1;
    $purid = 'SAQ' . str_pad(1, 8, '0', STR_PAD_LEFT);
} else {
    $val = $lst['PoID'] + 1;
    $purid = 'SAQ' . str_pad($val, 8, '0', STR_PAD_LEFT);
}

$getsalesorder=FETCH_all("SELECT `PoID` FROM `sales_order` WHERE `Sales_Quote`=? ",$_REQUEST['enquiryid']);
...
if (isset($_REQUEST['submit']))
{
    ...
    $msg = addsalesquote($invno, $billdate, $taxtype, $netpayamt, $subtotal, $subtaxtotal, $Item_name, $suppliergird, $Qty, $Pack_UOM, $Pack_qty, $Item_UOM, $Total_Qty, $Pack_Rate, $Item_Rate, $tax, $Taxable_Amt, $Net_Amt, $Tax_Amt, $purchase_detailid, $ip, $deliveryterms, $modeofshipment, $enquirycurrency, $modeofpayment, $paymentterms, $enquirytype, $vessel, $salesrep, $CusID, $id);
}
if(isset($_POST['saveItemMap']) && $_REQUEST['enquiryid']!=''){     
    $msg = addItemMapEnq($_POST['supplier_selected'],$_POST['prodIds'],'sales_quote',$_REQUEST['enquiryid'],$_POST['O_Enq_IM_ids']);
}
if(isset($_REQUEST['CreateRFQ']) && $_REQUEST['enquiryid']!=''){
    $msg = CreateRFQ('sales_quote',$_REQUEST['enquiryid'],$_REQUEST['chk']);
}
if(isset($_REQUEST['SendRFQ'])){
    $msg = SendRFQMail('sales_quote',$_REQUEST['enquiryid'],$_REQUEST['chk']);
}
if(isset($_REQUEST['ConvertoSalesQuote'])){
	 $msg = ConverttoSalesQuote($_REQUEST['enquiryid']);
}
?>
<style type="text/css">
...
<div class="content-wrapper">
    ...
    <section class="content-header">
        ...
        <ol class="breadcrumb">
            <li><a href="<?php echo $fsitename; ?>"><i class="fa fa-dashboard"></i> Dashboard</a></li>
            <li><a href="#"><i class="fa fa-money"></i> Process</a></li>
            <li><a href="#"><i class="fa fa-circle-o"></i> Master(s)</a></li>
            <li><a href="<?php echo $sitename; ?>process/sales_quote.htm"><i class="fa fa-circle-o"></i>Sales Quote</a></li>
            <li class="active"><?php
                if ($_REQUEST['enquiryid'] != '') {
                    echo 'Edit';
                } else {
    ...
    <section class="content">
        <form name="department" id="department" action="" method="post" enctype="multipart/form-data">
            <div class="box box-info">
                ...
                <div class="box-body">
                    <div id="sales_quote"><?php echo $msg; ?></div>
                    <div class="panel panel-info">
                        ...
                        <div class="panel-body">
                            <div class="row">
                                ...
                                <div class="col-md-3">
                                    <label>Quote Date<span style="color:#FF0000;"> *</span></label>
                                    <div class="input-group date">
                                        <div class="input-group-addon">
                                            <i class="fa fa-calendar"></i>
                                        </div>
                                        <input type="text" class="form-control pull-right usedatepicker" name="billdate" required="required"  value="<?php
                                        if (isset($_REQUEST['enquiryid']) && ( date('d-m-Y', strtotime(getsalesquote('PODate', $_REQUEST['enquiryid']))) != '01-01-1970')) {
                                            echo date('d-m-Y', strtotime(getsalesquote('PODate', $_REQUEST['enquiryid'])));
                                        }
                                        ?>" >
                                    </div>
                                </div>
                                <div class="col-md-3">
                                    ...
                                    <select name="taxtype" id="taxtype" class="form-control" onchange="univaerselcha()">
                                        <option value="0" <?php
                                        if (getsalesquote('TaxType', $_REQUEST['enquiryid']) == '0') {
                                            echo 'selected';
                                        }
                                        ?>>No Tax</option>
                                        <option value="1" <?php
                                        if (getsalesquote('TaxType', $_REQUEST['enquiryid']) == '1') {
                                            echo 'selected';
                                        }
                                        ?>>Inclusive Tax</option>
                                        <option value="2" <?php
                                        if (getsalesquote('TaxType', $_REQUEST['enquiryid']) == '2') {
                                            echo 'selected';
                                        }
                                        ?>>Excluded Tax</option>
                                    </select>
                                ...
                                <div class="col-md-3" id="suppliermode" >
                                    ...
                                    <div class="input-group date">
                                        <input type="text" readonly class="form-control"  name="customer" required="required" value="<?php echo getcustomer('Person', getsalesquote('CusID', $_REQUEST['enquiryid'])); ?>"/>
                                        <div class="input-group-addon">
                                ...
                                <div  style="display:none" id="supplierpanel">
                                    <input type="hidden" class="form-control"  name="CusID" id="CusID" required="required"value="<?php echo getsalesquote('CusID', $_REQUEST['enquiryid']); ?>"  />   
                                    <div class="col-md-3" >
                                    ...
                                    <div class="col-md-3" >
                                        ...
                                        <div class="col-md-9" >
                                            <label style="display: block !important ; font-size:20px;" id="compa_name"><?php echo getcustomer('Company', getsalesquote('CusID', $_REQUEST['enquiryid'])); ?></label>
                                            <label style="display: block !important ;" id="supp_code">Supplier_code</label>
                                            <label style="display: block !important ;  font-weight:400  !important ;" id="supp_address"><?php echo getcustomer('Address_1', getsalesquote('CusID', $_REQUEST['enquiryid'])); ?></label>
                                            <label style="display: block !important ; font-weight:400  !important ;" id="supp_state"><?php echo getcustomer('State', getsalesquote('CusID', $_REQUEST['enquiryid'])); ?></label>
                                            <label style="display: block !important ; font-weight:400  !important ;" id="supp_city"><?php echo getcustomer('City', getsalesquote('CusID', $_REQUEST['enquiryid'])); ?></label>
                                            <label style="display: block !important ; font-weight:400  !important ;" id="supp_county"><?php echo getcustomer('Country', getsalesquote('CusID', $_REQUEST['enquiryid'])); ?></label>
                                            <label style="display: block !important ; font-weight:400  !important ;" id="supp_postcode"><?php echo getcustomer('Postcode   ', getsalesquote('CusID', $_REQUEST['enquiryid'])); ?></label>
                                        </div>
                                    </div>
                                </div><br/>
                            </div>
                        ...
                        <div class="panel-body">
                            <div class="table-responsive">
                                <table class="table table-bordered" id="customFieldss">
                                    ...
                                    <tbody id="customFields">
                                        <?php
                                        if ($_REQUEST['enquiryid'] != '') {
                                            $getponumber = getsalesquote('PONumber', $_REQUEST['enquiryid']);
                                            $getitemvalue = $db->prepare("SELECT * FROM `sales_quote_details` WHERE `PoNum` = ? AND `PoID` =?");
                                            ...
                                            while ($fdepart = $getitemvalue->fetch()) {
                                                ...
                                                <tr valign="top" id="datarow<?php echo $key; ?>">
                                                    ...
                                                    <td>
                                                        <input type="number" style="border: 1px solid #f9f9f9;" class="form-control"  onkeyup="caltolqty(this.value, '<?php echo $key; ?>')"  placeholder="Qty" name="Qty[]" id="Qty<?php echo $key; ?>"  pattern="[0-9  .,:'()]{1,60}" data-id="<?php echo $key; ?>"  title="Allowed Characters (0-9A-Za-z .,:'()]{1,60})" value="<?php echo $fdepart['Qty']; ?>" />
                                                    </td>
                                                    <td>
                                                    ...
                                                    <td>
                                                        <input type="text" style="border: 1px solid #f9f9f9; text-align: right;" class="form-control"  name="Net_Amt[]" id="Net_Amt<?php echo $key; ?>"  pattern="[0-9 A-Z a-z .,:'()]{2,60}" title="Allowed Characters (0-9A-Za-z .,:'()]{2,60})"  data-id="<?php echo $key; ?>" value="<?php echo number_format((float) $fdepart['Amount'], 2, '.', ''); ?>" readonly="readonly" />
                                                    </td>    
                                                    <td valign="middle">
                            ...
                            <div class="row">
                                <div class="col-md-12">
									...
									<?php if($_REQUEST['enquiryid']!='') { ?> <a id="ItemMap" onclick="loaditemmap()" class="btn btn-sm btn-info"  aria-hidden="true" data-toggle="modal" data-target="#ItemMapModal" >Item Mapping</a>
								   <a onclick="loaditemrfqs()" class="btn btn-sm btn-info"  aria-hidden="true" data-toggle="modal" data-target="#ItemRFQModal" >Send RFQ</a><?php } ?>
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-6">
                                        ...
                                        <div class="col-md-12">
                                            <span >
                                                ...
                                                <span id='subtotaldisplay'><?php
                                                    if ($_REQUEST['enquiryid'] != '') {
                                                        echo number_format((float) getsalesquote('SubTotal', $_REQUEST['enquiryid']), 2, '.', '');
                                                    }
                                                    ?></span>
                                        ...
                                        <div class="col-md-12">
                                            <span>
                                                ...
                                                <span id='taxdisplay'><?php
                                                    if ($_REQUEST['enquiryid'] != '') {
                                                        echo number_format((float) getsalesquote('SubTaxTotal', $_REQUEST['enquiryid']), 2, '.', '');
                                                    }
                                                    ?></span>
                                        ...
                                        <div class="col-md-12">
                                            <span >
                                                ...
                                                <span id='NetAmtdisplay'><?php
                                                    if ($_REQUEST['enquiryid'] != '') {
                                                        echo number_format((float) getsalesquote('Total', $_REQUEST['enquiryid']), 2, '.', '');
                                                    }
                                                    ?></span>
                ...
                <div class="box-footer">
                    <div class="row">
                        <div class="col-md-4">
                            <a href="<?php echo $sitename; ?>process/sales_quote.htm">Back to Listings page</a>
                        </div>
                        <div class="col-md-4 text-center">
                            <?php if (($_REQUEST['enquiryid'] != '')&&($getsalesorder['PoID']=='')) { ?>
                                <button type="button" name="convert" id="convert" onclick="convertsalesorder('<?php echo $_REQUEST['enquiryid']; ?>');" class="btn btn-success text-center">CONVERT TO SALES ORDER</button>
                            <?php } ?>
                        </div>
...
<script>
    ...
    function loaditemgrid() {
        ...
        $('#normalexamples1').dataTable({
            ...
            "searching": true,
            "destroy": true,
            "sAjaxSource": "<?php echo $fsitename; ?>pages/dataajax/purchase_table.htm?types=itemtable&supplierid=" + supplierid
        });
    }
    ...
    function loadgrid(str) {
        ...
        $('#normalexamples').dataTable({
            ...
            "searching": true,
            "destroy": true,
            "sAjaxSource": "<?php echo $fsitename; ?>pages/dataajax/purchase_table.htm?types=customertable&sqlitemid=" + setfinalarray
        });
    }

    function convertsalesorder(a)
    {
        var ponumber = $("#invno").val();
        var a = a;
        $.post("<?php echo $fsitename; ?>pages/master/userajax.php", {converttosalesorder: a, ponumber:ponumber},
        function (data)
        {
            $("#convert").hide();
            $("#sales_quote").html('<div class="alert alert-success alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button><h4><i class="icon fa fa-check"></i>Successfully Converted</h4></div>');
        });
    }
    ...
    function addgriddata(a) {
        ...
        if (a != '') {
            $.post('<?php echo $fsitename; ?>pages/master/userajax.php', {getCustomerDetails: a}, function (data) {
                if (data['image'] === '') {
    ...
    function addrow(itemid) {
        ...
        $.post("<?php echo $fsitename; ?>pages/master/userajax.php", {supplierfull: 'ALL', findkeyid: findid, selectitem: itemid},
                function (data)
                {
    ...
    function selectitem(value, id) {
        setTimeout(function () {
            if (value !== '') {
                $.post('<?php echo $fsitename; ?>pages/master/userajax.php', {itemkeyid: value}, function (data) {
    ...
    function remove(b) {
        ...
        if (confirm("Please confirm you want to Delete this Item(s)"))
        {
            if (getv1 != 'NEW') {
                ...
                $("#addbutton").attr("disabled", "disabled");
                $("#submit").attr("disabled", "disabled");
                $.post("<?php echo $fsitename; ?>pages/master/userajax.php", {deledtered: sentdeldata, tablename: senttablename, primarykey: sentfield},
                        function (data)
                        {
                            $("#addbutton").removeAttr("disabled", "disabled");
                            $("#submit").removeAttr("disabled", "disabled");
    ...
    function showtrans(a) {
        ...
        $.post('<?php echo $fsitename; ?>pages/master/userajax.php', {seetrans: '1', supp_id: a}, function (data) {
            $('#st').html(data);
        });
    }
	
	function loaditemmap(){
		$.post('<?php echo $sitename; ?>pages/master/userajax.php', {show_selec_prod_sq: '1', sales_quote_id: '<?php echo $_REQUEST['enquiryid']; ?>'}, function (data) {
	...
	function loaditemrfqs(){
		$.post('<?php echo $sitename; ?>pages/master/userajax.php', {show_selec_supp_sq: '1', sales_quote_id: '<?php echo $_REQUEST['enquiryid']; ?>'}, function (data) {
